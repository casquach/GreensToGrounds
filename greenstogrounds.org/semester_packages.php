...
HERE;
		...
		//$result = mysql_query($sql, $conn);
		$sql = "SELECT paid_orders.timestamp,paid_orders.name,paid_orders.Comments, paid_orders.cost,paid_orders.box_content FROM paid_orders WHERE (paid_orders.package !=  'None') AND (timestamp LIKE '%09/__/2019%' OR timestamp LIKE '%09/__/2019%' OR timestamp LIKE '%09/__/2019%') AND box_content LIKE '%Season%' AND name != '' AND lower(paid_orders.name) NOT LIKE '%test%' ORDER BY `paid_orders`.`timestamp` DESC";
