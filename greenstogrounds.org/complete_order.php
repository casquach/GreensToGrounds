...
if(isset($_POST['name']))
{
  ...
  //check emptiness
	...
	if(empty($errors)) {
		...
		$sql = "INSERT INTO unpaid_orders VALUES (NULL, '$timestamp', '$name', '$email', '$phone_num', '$year', '$cost', '$package', '$box_content', 'yes', '$comments')";
		$result = $mysqli->query($sql);
		$sql = "INSERT INTO paid_orders VALUES (NULL, '$timestamp', '$name', '$email', '$phone_num', '$year', '$cost', '$package', '$box_content', 'yes', '$comments')";
		$result = $mysqli->query($sql);
...
<!-- 						<input type='radio' name='box_content' value='Wine Night Box ($15)'>Wine Night Box ($15)<br />
 -->					<div class="alert">***Please be aware that, due to the local, organic nature of our food, some last-minute swaps may be necessary to accommodate unexpected changes in availability</div>
 						...
 						<!--disclaimer-->	
 						<div class= "disclaimer">Please note that if you are getting a Semester Package you do not need to need to select a box</div>
						<!--comments-->

						<div class="order-form-header">Comments:</div>
						<br />Please specify any dietary restrictions so we may accomodate you<br /><textarea name="comments" rows=4 cols=30 value='<?php echo htmlentities($comments) ?>'></textarea>
						<!--captcha-->
						<div id="capcha">
