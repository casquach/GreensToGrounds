...
HERE;
		...
		//$result = mysql_query($sql, $conn);
		$sql = "SELECT paid_orders.name,paid_orders.cost,paid_orders.box_content FROM paid_orders WHERE timestamp LIKE '%09/16/2019%' OR timestamp LIKE '%09/17/2019%' OR timestamp LIKE '%09/18/2019%'  AND name != '' AND lower(paid_orders.name) NOT LIKE '%test%' ORDER BY `paid_orders`.`timestamp` DESC";
