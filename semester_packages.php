...
<html>
	...
	<body>
		<center>
			...
			<h1 style="color:green; border-style:solid">Semester Packages</h1>
		</center>
...
HERE;
		...
		//$result = mysql_query($sql, $conn);
		$sql = "SELECT paid_orders.timestamp,paid_orders.name,paid_orders.Comments, paid_orders.cost,paid_orders.box_content FROM paid_orders WHERE (paid_orders.package !=  'None') AND (timestamp LIKE '%08/__/2018%' OR timestamp LIKE '%09/__/2018%' OR timestamp LIKE '%10/__/2018%' OR timestamp LIKE '%08/__/2018%' OR timestamp LIKE '%09/__/2018%') AND box_content LIKE '%Season%' AND name != '' AND lower(paid_orders.name) NOT LIKE '%test%' ORDER BY `paid_orders`.`timestamp` DESC";
		...
		print "<div class='component'><table border=1 >\n";
		print "<thead style='background-color:white;'><tr>\n";
		print "<th>timestamp</th><th>Name</th><th>Cost</th><th>Box Content</th><th>Comments</th>";
		print "</tr></thead>\n\n<tbody>";
